...
- (void)testCharSequenceLength {
  ...
  XCTAssertEqual(len, (NSUInteger) 5,
                 @"char sequence length should be 5, but was %lu",
                 (unsigned long)len);
}
...
- (void)testSplit {
  ...
  XCTAssertEqual((NSUInteger) 3, [parts length], @"Wrong number of parts.");
  ...
  XCTAssertEqual((NSUInteger) 5, [parts length], @"Wrong number of parts.");
  ...
  // Regular expression.
  parts = [@"abba" split:@"[b]+"];
  XCTAssertEqual((NSUInteger) 2, [parts length], @"Wrong number of parts.");
  XCTAssertEqualObjects(@"a", [parts objectAtIndex:0], @"Wrong part.");
  XCTAssertEqualObjects(@"a", [parts objectAtIndex:1], @"Wrong part.");
  ...
  // Empty string.
  parts = [@"" split:@"\\s+"];
  XCTAssertEqual((NSUInteger) 1, [parts length], @"Wrong number of parts.");
  XCTAssertEqualObjects(@"", [parts objectAtIndex:0], @"Wrong part.");
